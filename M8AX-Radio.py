...
Mi OnCyber - ((( ★ https://oncyber.io/@m8ax ★ )))\n"""
    ...
    if rate == "0":
        meteda2 = f"{factores}\nEl Audio Se Ha Grabado Con Codec Libopus, A Los Hercios Originales, Con Un Bitrate Máximo De {calidad}, Un Bitrate Mínimo De {minrate} Y Con El VBR En {vbr}. El Nivel De Compresión Ha Sido {com_level} De 10 Y Se Han Usado {nucleos_cpu} Núcleos De La CPU. El Directorio De Grabación Ha Sido {download_path} Y Se Ha Grabado En Esta Fecha: {timestamp} Además Se Ha Aplicado Este Filtro: {vamos}."
    else:
        meteda2 = f"{factores}\nEl Audio Se Ha Grabado Con Codec Libopus, A {rate} Hz, Con Un Bitrate Máximo De {calidad}, Un Bitrate Mínimo De {minrate} Y Con El VBR En {vbr}. El Nivel De Compresión Ha Sido {com_level} De 10 Y Se Han Usado {nucleos_cpu} Núcleos De La CPU. El Directorio De Grabación Ha Sido {download_path} Y Se Ha Grabado En Esta Fecha: {timestamp} Además Se Ha Aplicado Este Filtro: {vamos}."
    if sitags == "no":
        metedato = ""
        meteda2 = ""
    if opcion == "1":
